...
func TestClusterResource_Schema_HasProviderAttribute(t *testing.T) {
	r := &ClusterResource{}
	resp := &resource.SchemaResponse{}

	r.Schema(t.Context(), resource.SchemaRequest{}, resp)

	attr, ok := resp.Schema.Attributes["provider"]
	require.True(t, ok, "schema must have 'provider' attribute")
	assert.True(t, attr.IsOptional(), "provider should be optional")
	assert.False(t, attr.IsRequired(), "provider should not be required")
	assert.False(t, attr.IsComputed(), "provider should not be computed")
}
